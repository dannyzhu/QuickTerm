...
enum ConfigSchema {
    ...
    static let keys: [ConfigKeySpec] = [
        ...
        ConfigKeySpec(.appearance, "pane-padding", .int(min: 0, max: 32), default: .int(8),
                      labelZH: "终端内边距", labelEN: "Terminal padding",
                      helpZH: "pane 内终端四边留白（pt，0–32；默认 8）",
                      helpEN: "terminal padding in pt, 0–32 (default 8)",
        ...
        ConfigKeySpec(.agents, "info-strip", .bool, default: .bool(true),
                      ...
                      helpZH: "每个终端 pane 顶部常驻一条独立状态带（有 agent 或设过标题时才画内容；与内边距无关）",
                      helpEN: "a dedicated status strip reserved at the top of every terminal pane (drawn only when there is an agent or a set title; independent of pane-padding)"),
        ConfigKeySpec(.agents, "strip-height", .int(min: 0, max: 40), default: .int(16),
                      labelZH: "状态带高度", labelEN: "Status strip height",
                      helpZH: "状态带高度（pt，0–40；0 = 不预留，回到画在内边距里的老行为）",
                      helpEN: "status strip height in pt, 0–40 (0 = reserve nothing)"),
        ConfigKeySpec(.agents, "strip-font-size", .int(min: 8, max: 24), default: .int(11),
                      labelZH: "状态带字号", labelEN: "Status strip font size",
                      helpZH: "状态带文字大小（pt，8–24）",
                      helpEN: "status strip text size in pt, 8–24"),
