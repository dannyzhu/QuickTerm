...
| 布局 | dwindle + 可切换 scrolling（`column_width = 0.49`） | **scrolling 默认**（列宽 (1−2×6%)/列数 = 0.44，每侧露边 6%），dwindle 保留，`Cmd+L` 切换 |
...
- 列宽因子默认 `(1 − 2×peek) / visible-columns`，`peek = 6%`（默认 2 列 → 0.44）；可调 0.25–0.90，键盘 ±0.05。
  Omarchy 原值 0.49（总露边 2%）盖不住 gap+边框+终端内边距，露出的只是空壳；6% 一侧能看到真实内容。
...
**引擎配置链（低 → 高）**：① libghostty 内置默认 → ② `~/.config/ghostty/config`（原生规则，含递归 include）→ ③ QuickTerm 覆盖文件 `~/Library/Application Support/QuickTerm/engine-overlay.conf`（`ThemeManager.overlayExtra()` 生成：`window-vsync = false`、`window-padding-x/y`、`background-opacity`、`unfocused-split-opacity`、配色与 palette）→ ④ `[ghostty]` 段（物理上追加在 ③ 末尾）。`theme = "ghostty"` 时 ③ 只注 padding 与（关闭透明时的）不透明覆盖。`~/.config/ghostty/config` 无监听，改后需重启或触发一次 overlay 重写。**注意**：引擎在 overlay 之后才处理 `config-file` 递归包含，因此写在被包含文件里的键会压过第 3、4 层。
