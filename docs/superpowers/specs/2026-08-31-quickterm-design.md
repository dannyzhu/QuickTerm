...
**修订 v6**（2026-09-01）：`pane-padding` 配置项（pane 内终端四边留白，默认 3pt，0–32，注入引擎 window-padding）；scrolling 几何补全（参照 Omarchy）：**单列升格占满视口**、**内容总宽 ≤ 视口时整组居中**（两列左右间隙相等），仅溢出时进入最小滚动+露边模式
