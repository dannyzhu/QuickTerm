...
| 布局 | dwindle + 可切换 scrolling（`column_width = 0.49`） | **scrolling 默认**，dwindle 保留，`Cmd+L` 切换 |
...
- 列宽因子默认 `0.98 / visible-columns`（默认 2 列 → 0.49，留 2% 露边）；可调 0.25–0.90，键盘 ±0.05。
- **填充模式**：名义总宽装得下时按比例放大填满（单列满屏、两列左中右等隙）；溢出才按名义宽度**最小滚动 + 露边**。视口跟随 0.15s easeOut。
