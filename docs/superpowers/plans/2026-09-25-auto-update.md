...
1. Build the older app: `SIGN_IDENTITY="Developer ID Application: Hangzhou Wujie Tongshun Technology Co., Ltd (PS7PJY4CYN)" E2E_BUNDLE_ID=dev.danny.quickterm.e2e E2E_BUILD_NUMBER=9001 scripts/make-release.sh --first-release` then `cp -R build/release/Build/Products/Release/QuickTerm.app build/e2e-old.app` (the DMG it produced is not needed).
...
4. Launch the older app as a second instance: `open -n build/e2e-old.app --args --update-feed-url https://github.com/<repo>/releases/download/e2e-<date>/appcast.xml --state-file /tmp/e2e-state.json --control-socket /tmp/e2e.sock --config-file /tmp/e2e-config.toml` (a Release build honours no feed override by design — for the E2E use a Debug build of both, built with `xcodebuild -configuration Debug PRODUCT_BUNDLE_IDENTIFIER=dev.danny.quickterm.e2e CURRENT_PROJECT_VERSION=9001`, signed inside-out with the Developer ID by hand: `QuickTerm.debug.dylib`, the four Sparkle nested items, `Sparkle.framework`, then the app without `--options runtime`).
5. Walk: **Check for Updates…** → icon → sheet → **Install and Relaunch** → the app quits without relaunching (override) → `open -n build/e2e-old.app --args …` again with the same arguments → `About QuickTerm` shows build 9002 and the scratch layout is restored. Then, with `install = true` in `/tmp/e2e-config.toml`, the staged path: the icon turns to `power.circle` after the background download; **Restart Now** and a plain quit both install it.
...
7. Teardown: `defaults delete dev.danny.quickterm.e2e`, `gh release delete e2e-<date> --yes --cleanup-tag`, `rm -rf build/e2e-old.app build/e2e-new.dmg /tmp/e2e-*`.
