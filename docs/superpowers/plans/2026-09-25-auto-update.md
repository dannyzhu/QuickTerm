...
5. Walk: **Check for Updates…** → icon → sheet → **Install and Relaunch** → the app quits and Sparkle relaunches it; the relaunched instance takes its overrides back from its defaults domain (spec §8: a Sparkle relaunch carries no arguments, and `updaterShouldRelaunchApplication` must stay true because a NO aborts the install) → `About QuickTerm` shows build 9002 and the scratch layout is restored. Then the staged path: the installed app is already build 9002 after the first walk, so copy the 9001 app to `/Applications/QuickTerm E2E.app` afresh (or build a 9003 and re-upload the feed), set `install = true` in `/tmp/e2e-config.toml`, and launch again: the icon turns to `power.circle` after the background download; **Restart Now** and a plain quit both install it. `log stream --level info --predicate 'subsystem == "dev.danny.quickterm.e2e" AND category == "updates"'` shows every state transition.
