...
- **Scrolling canvas by default** — each workspace is an endless horizontal strip of columns (Hyprland `scrolling` layout, `column_width = 0.49`). New terminals open to the right of the focused column; the viewport follows focus with minimal scrolling and neighbouring columns peek in at the edges. Classic **dwindle** tiling is one `Cmd+L` away.
